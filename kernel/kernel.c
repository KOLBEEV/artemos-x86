...
static uint16_t vga_entry(unsigned char symbol, uint8_t color)
{
    return (uint16_t) symbol | (uint16_t) color << 8;
}
...
static void terminal_putchar(char symbol)
{
    if (symbol == '\n') {
        terminal_column = 0;
        terminal_row++;
    ...
    terminal_buffer[index] = vga_entry(symbol, terminal_color);
...
void kernel_main(void)
{
    ...
    terminal_write("Welcome to ArtemOS!\n");
